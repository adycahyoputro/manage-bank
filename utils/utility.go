...
func CheckError(message string, err error) dto.Response {
	...
	if err.Error() == "first name is required" || err.Error() == "last name is required" || err.Error() == "email is required" || err.Error() == "password is required" || err.Error() == "destination account is required" || err.Error() == "amount is required" || err.Error() == "amount must be positive amount" || err.Error() == "wrong password" {
		result = dto.Response{
